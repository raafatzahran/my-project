-- Queue: KivenlahtiOflow,KivenlahtiBf groupn by: agent, agent: HansL, choose: Answered
...
GROUP BY ql.callId 
HAVING state IS NOT NULL AND state IN ('answer') 
	AND agent IN ('HansL') 
...
WHERE  	ql.callid = '1517408131.2152' AND
		...
		ql.EVENT IN ('ENTERQUEUE','DID','CONNECT','COMPLETEAGENT','COMPLETECALLER','TRANSFER','EXITWITHKEY','ABANDON','EXITEMPTY','EXITWITHTIMEOUT') AND 
		q.queuename IN ('8003','8001') AND  -- ql.queuename IN ('8001') AND 
		q.TIME > '2018-01-01 01:00:00.0' AND 
		q.TIME < '2018-02-01 00:59:00.0' ;
-- ----------------------------------------
select * 
