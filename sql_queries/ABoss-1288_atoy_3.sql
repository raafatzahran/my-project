-- Queue: KivenlahtiOflow, groupn by: Queue, choose: Answered
-- if we compare the row with callid='1517408131.2152' with the correspond row in (total column) in web-agent
...
GROUP BY ql.callId 
HAVING state IS NOT NULL AND state IN ('answer') -- ('noAnswer','timeout') 
...
WHERE  	ql.callid = '1517408131.2152' AND
