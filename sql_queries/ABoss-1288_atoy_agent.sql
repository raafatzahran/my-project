-- Queue: {KivenlahtiOflow:8003,KivenlahtiBf:8001, VaristoOflow:8033} or {all queues: q.queuename NOT LIKE '1____' } 
-- group by: agent, {agent: HansL, choose: Answered: 204} or {agent: RitvaL, choose: Answered: 118}
SELECT 
	MAX(CASE ql.EVENT 
...
WHERE  	q.EVENT='ENTERQUEUE'  AND 
		...
		q.queuename IN ('8001','8003','8033') AND -- NOT LIKE '1____'   IN ('8001','8003','8033')
		q.TIME > '2018-01-01 01:00:00.0' AND 
		q.TIME < '2018-02-01 00:59:00.0' 
GROUP BY ql.callId,ql.queuename
HAVING state IS NOT NULL  AND state IN ('answer') -- ('timeout', 'noAnswer') -- ('answer')
	AND agent IN ('HansL')  -- ('HansL') ('RitvaL') 
