...
HAVING state IS NOT NULL AND 
	state IN ('answer')  AND
	peerNumber IN (966200302)
ORDER BY callDate LIMIT 0,50;

select * from tmg_asterisk.queue_log where callid=1518024278.7544;

SELECT * from tmg_customer.dialHistory where originatingCallId=1518025169.7798;

SELECT * from tmg_customer.dialHistory where customerId in ('+351966200302');
